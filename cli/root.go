// Package cli assembles the imf command tree from the imf
...
// NewApp assembles the kit application from the imf domain. The
...
//
// To add a command, declare it in imf/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := imf.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(imf.Domain{}).Register(app)
