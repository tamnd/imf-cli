...
describes what users get. Pick a format with `-o`, or let imf choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
imf <command> -o table   # aligned columns for reading
imf <command> -o jsonl   # one JSON object per line, for piping
imf <command> -o json    # a single JSON array
imf <command> -o csv     # spreadsheet friendly
imf <command> -o tsv     # tab-separated
imf <command> -o url     # just the URL column
imf <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
imf <command> --fields id,title,url
```
...
```bash
imf <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
imf <command>            # a table, because this is a terminal
imf <command> | wc -l    # JSONL, because this is a pipe
```
